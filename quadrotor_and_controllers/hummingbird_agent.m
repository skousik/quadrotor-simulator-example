classdef hummingbird_agent < rigid_body_agent_SE3
properties
...
methods
%% constructor
    function A = hummingbird_agent(varargin)
        % agent = hummingbird_agent()
        %
        ...
        A@rigid_body_agent_SE3('body_mass',m,'body_inertia_matrix',J,varargin{:}) ;
...
%% move (now it saves the desired trajectory)
...
%% dynamics
    function zd = dynamics(A,t,z,R,T_ref,U_ref,Z_ref)
        ...
        % % FOR DEBUGGING:
        % if any(isnan(U(:)))
        %     dbstop in quadrotor_agent at 190
        % end
...
%% plotting
    ...
    function plot_body(A,R,z)
        ...
        if check_if_plot_is_available(A,'body')
            ...
            body_patch_data =  patch('Faces',FB1,'Vertices',VB,...
                               'FaceAlpha',A.plot_input_data.body_opacity,...
                               'FaceColor',A.plot_input_data.body_face_color,...
                               'EdgeColor',A.plot_input_data.body_edge_color) ;
    ...
    function make_plot_input_data(A)
        ...
        [FB1,FB2,VB] = make_hummingbird_body() ;
        ...
        % set default opacity and color values
        A.plot_input_data.body_face_color = [0.7 0.7 1] ;
        A.plot_input_data.body_edge_color = [0 0 1] ;
        A.plot_input_data.body_opacity = 1 ;
    end
